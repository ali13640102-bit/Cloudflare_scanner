...
def worker_round_1():
    while not ip_queue.empty():
        ip = ip_queue.get()
        ping = ping_ip(ip, TIMEOUT_ROUND_1)
        if ping is not None:
...
def worker_round_2():
    while not ip_queue.empty():
        ...
        second_ping = ping_ip(ip_info['ip'], TIMEOUT_ROUND_2)
...
def main():
    ...
    # وارد کردن تمام رنج‌های داخل فایل به صف بدون هیچ محدودیت یا انتخاب تصادفی
    for line in lines:
        try:
    ...
    if sorted_ips:
        ranges_str = ", ".join(lines[:5])
        if len(lines) > 5:
            ranges_str += " و..."
            
        msg = f"<b>اسکن کامل رنج‌های ({ranges_str}) به پایان رسید!</b>\n\n"
